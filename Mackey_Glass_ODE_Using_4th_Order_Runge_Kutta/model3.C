...
double XXF[resSize][sampleTimes];
double WoutXA[outSize];

double abs(double x){ return sqrt(x*x); }

ifstream readin("mglass.dat");
ofstream prtx("model3.dat");
ofstream prtvalid("model3_valid.dat");
ofstream prteigs("model3_eigs.dat");
...
int main(){
	...
	double xA[resSize], diff,WAx[resSize];
	int i,j,k,p,t,info;
	const int lwork=8*resSize;
	char jobvl='N', jobvr='N';
	double valr[resSize],vali[resSize],work[lwork],dumv[2];
...
// loop time sample -------------------------------------------------------------------------------
	...
	t++;
        ...
        for(i=0;i<resSize;i++){ xA[i]=xold[i]; }
...
// This last segment is for PART 3 of the assignment------------------------------------------------------------

	int K;
        makeWA(W,Win,Wout); 			//initializing the matrix WA = W + WinWout
       	t=transientTimes+sampleTimes;		//starting the time evolution from the last point in the training phase
	K=t-transientTimes-sampleTimes; 	// time adjustment	

// loop time sample WA Autonomous trained network starting from the last point in the training phase ------------

        while(t<transientTimes+sampleTimes+sampleTimes){
        //for(i=0;i<6;i++){ prtx << xold[i] << " "; } prtx << endl;

        readin >> tin >> u[0] >> uintau;

	MultMatVec1(WA,xold,WAx);

        for(i=0;i<resSize;i++){
                xnew[i]=(1.0-alp)*xold[i]+alp*tanh(WAx[i]);
                xold[i]=xnew[i];
		XXF[i][K]=xnew[i];
        }

	for(i=0;i<outSize;i++){ YY[i][K]=u[i]; }
        t++;
        }       // This ends the while loop for loop time sample


// constructing Wout*x(t_f+1) for autonomous network -----------------------------------------------------

        t=transientTimes+sampleTimes;
        diff=0.0;
        while(t<transientTimes+sampleTimes+sampleTimes){
                for(i=0;i<resSize;i++){ xold[i]=XXF[i][K]; }
                cout << "Test Wout " << t << " ";
                s = 0.0;
                for(i=0;i<outSize;i++){
                        WoutXA[i]=0.0;
                        for(j=0;j<resSize;j++){
                                WoutXA[i]+=Wout[i][j]*xold[j];
                        }
                prtvalid << t  << " " << WoutXA[i] << " ";
                prtvalid << YY[i][K] << " " << abs(YY[i][K]-WoutXA[i]) << endl;
                s=(YY[i][K]-WoutXA[i])*(YY[i][K]-WoutXA[i]);
                }
                diff=sqrt(s);
                cout   << " " << diff << endl;
                t++;
